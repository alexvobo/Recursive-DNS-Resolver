...
def populate_dns_key():
    ...
    file_names = ["PROJ3-DNSTS1.txt", "PROJ3-KEY1.txt"]
    try:
        ...
        print("[TS1]: Error. File {} not found".format(file.name))
        exit()
    finally:
        file.close()
...
def server_AS(port):
    try:
        ...
        print("[TS1_AS]: TS Server socket for AS created")
        server_binding = ('', port)
        ss.bind(server_binding)
        ss.listen(1)
        host = socket.gethostname()
        print("[TS1_AS]: Server host name is: {}".format(host))
        host_ip = (socket.gethostbyname(host))
        print("[TS1_AS]: Server IP address is: {}".format(host_ip))
        print("[TS1_AS]: Server port is: {}".format(port))
    except socket.error as err:
        print('[TS1_AS]: Socket open error: {}\n'.format(err))
        exit()

    while True:
        as_server, address = ss.accept()
        print("[TS1_AS]: Got a connection request from as_server {}".format(address))
        try:
            ...
            while data_from_as_server or 0:
                ...
                print("[TS1_AS]: as_server sent {} | Sending {} to as_server {}".format(data_from_as_server, data,
                                                                                        address))
...
def server_client(port):
    ...
    try:
        ss = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        print("[TS1_C]: TS Server socket for Client created")
        server_binding = ('localhost', port)
        ss.bind(server_binding)
        ss.listen(1)
        host = socket.gethostname()
        print("[TS1_C]: Server host name is: {}".format(host))
        host_ip = (socket.gethostbyname(host))
        print("[TS1_C]: Server IP address is: {}".format(host_ip))
        print("[TS1_C]: Server port is: {}".format(port))
    except socket.error as err:
        print('[TS1_C]: Socket open error: {}\n'.format(err))
        exit()

    while True:
        client, address = ss.accept()
        print("[TS1_C]: Got a connection request from client {}".format(address))
        try:
            ...
            while data_from_client or 0:
                ...
                print("[TS1_C]: client sent {} | Sending {} to client {}".format(data_from_client, data, address))
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 3:
        print("Error.\n Syntax: python ts1.py ts1ListenPort_a ts1ListenPort_c")
        exit()

    ts1ListenPort_a = test_port(sys.argv[1])
    ts1ListenPort_c = test_port(sys.argv[2])

    populate_dns_key()

    t1 = threading.Thread(name='server_AS', target=server_AS, args=(ts1ListenPort_a,))
    t1.start()
    t2 = threading.Thread(name='server_client', target=server_client, args=(ts1ListenPort_c,))
    t2.start()
